...
const defualtProps: InputProps = {
  onChange: jest.fn(),
  placeholder: 'test-input',
};
describe('test Input component', () => {
  it('should render the correct default Input', () => {
    const wrapper = render(<Input {...defualtProps} />);
    const testNode = wrapper.getByPlaceholderText(
      'test-input'
    ) as HTMLInputElement;
    expect(testNode).toBeInTheDocument();
    expect(testNode).toHaveClass('kk-input-inner');
    fireEvent.change(testNode, { target: { value: '23' } });
    expect(defualtProps.onChange).toHaveBeenCalled();
  ...
  it('should render different input sizes on size property', () => {
    ...
    const testContainer = wrapper.container.querySelector('.kk-input-wrapper');
  ...
  it('should render prepend and append element on prepend/append property', () => {
    ...
    const testContainer = container.querySelector('.kk-input-wrapper');
